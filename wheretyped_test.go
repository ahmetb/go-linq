...
func TestWhereT_PanicWhenParameterInTypeDoesntMatch(t *testing.T) {
	defer func() {
		r := recover()
		t.Log(r)
		if r == nil {
	...
	var r []int
	From([]int{1, 1, 1, 2, 1, 2, 3, 4, 2}).WhereT(func(item string) bool { return len(item) > 0 }).ToSlice(&r)
}
...
func TestWhereIndexed_PanicWhenParameterTypesDoesntMatch(t *testing.T) {
	defer func() {
		r := recover()
		t.Log(r)
		if r == nil {
