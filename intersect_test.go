...
func TestIntersect(t *testing.T) {
	...
	if q := legacyFrom(input1).Intersect(legacyFrom(input2)); !testQueryIteration(q, want) {
...
func TestIntersectBy(t *testing.T) {
	...
	if q := legacyFrom(input1).IntersectBy(legacyFrom(input2), func(i any) any {
		return i.(int) % 2
...
func TestIntersectByT_PanicWhenSelectorFnIsInvalid(t *testing.T) {
	mustPanicWithError(t, "IntersectByT: parameter [selectorFn] has a invalid function signature. Expected: 'func(T)T', actual: 'func(int,int)int'", func() {
		legacyFrom([]int{5, 7, 8}).IntersectByT(legacyFrom([]int{1, 4, 7, 9, 12, 3}), func(i, x int) int {
			return i % 2
		})
	})
}
