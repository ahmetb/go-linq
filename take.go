...
// Take returns a specified number of contiguous elements from the start of a
// collection.
func (q QueryG[T]) Take(count int) QueryG[T] {
	return QueryG[T]{
...
// TakeWhile returns elements from a collection as long as a specified condition
// is true, and then skips the remaining elements.
func (q QueryG[T]) TakeWhile(predicate func(T) bool) QueryG[T] {
	return QueryG[T]{
		Iterate: func() IteratorG[T] {
			next := q.Iterate()
			done := false
...
// the element within collection. The second argument represents the element to
// test.
func (q QueryG[T]) TakeWhileIndexed(predicate func(int, T) bool) QueryG[T] {
	return QueryG[T]{
		Iterate: func() IteratorG[T] {
			next := q.Iterate()
			done := false
